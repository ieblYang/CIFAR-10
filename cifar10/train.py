...
def model(image, keep_prob=0.8, is_training=True):  #定义一个模型
    
	batch_norm_params = {
        "is_training": is_training,
        "epsilon":1e-5,		#防止batch_norm归一化的时候除0
        "decay":0.997,		#衰减系数
        'scale':True,
        'updates_collections':tf.GraphKeys.UPDATE_OPS	#对batch_norm参数收集
    }

    with slim.arg_scope(	#定义优化器
        [slim.conv2d],
        weights_initializer = slim.variance_scaling_initializer(),	#权值初始化
        activation_fn = tf.nn.relu,	#激活函数
        weights_regularizer = slim.l2_regularizer(0.0001),	#正则化方法 l2正则 权值0.0001
        normalizer_fn = slim.batch_norm,
        normalizer_params = batch_norm_params):	#指定batch_norm参数
        with slim.arg_scope([slim.max_pool2d], padding="SAME"):			#保证每次池化之后，尺寸是等比例变化的
            net = slim.conv2d(image, 32, [3, 3], scope='conv1')			#卷积1
            net = slim.conv2d(net, 32, [3, 3], scope='conv2')			#卷积2 输入为卷积1net
            net = slim.max_pool2d(net, [3, 3], stride=2, scope='pool1')	#池化，进行两倍下采样
            ...
            slim.dropout(net, keep_prob)			#对神经元进行正则化
            net = slim.fully_connected(net, 10)
    return net #10 dim vec 10维向量


def loss(logits, label):  # 交叉熵损失函数,输入(logits预测出的概率分布值,label),输出 loss
